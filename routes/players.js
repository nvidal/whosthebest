...
//GET devuelve ordenado las posiciones por semana para 
// mostrarlo en la grafica
router.get('/historial/:id', function(req, res){
	PlayerHist = mongoose.model('PlayersHist');
	PlayerHist.find({playerId : req.params.id}).sort({ fecha : -1})
		.limit(config.limites.limiteGrafica).exec(function(err, hist){
		if (err) throw err;
